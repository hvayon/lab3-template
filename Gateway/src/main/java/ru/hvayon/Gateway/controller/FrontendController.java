...
import ru.hvayon.Gateway.auth.AuthRequest;
//import ru.hvayon.Gateway.auth.UserInfoResponse;
...
public class FrontendController {
    ...
    public String login(@ModelAttribute AuthRequest token, Model model) {
        authService.authorize(token.getUsername(), token.getPassword());
        String username = authService.auth("Bearer " + authService.getAuthToken()).getPrincipal();
    ...
    @PostMapping(value = "/auth", params = "action=register") // мб возможность редактирования добавить
    public String register(@ModelAttribute AuthRequest token, Model model) {
        UserInfoRequest userInfo = new UserInfoRequest();
        userInfo.setUsername(token.getUsername());
        userInfo.setPassword(token.getPassword());
        userInfo.setEmail(token.getUsername() + "@gmail.com");
